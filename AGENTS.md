...
- Default Hub Space id is **`OpenKotOR/site`** → static app URL **`https://openkotor-site.static.hf.space/`** (Hub hostnames are always `{org_slug}-{space_slug}.static.hf.space`). **`https://openkotor.static.hf.space/`** is not assignable to a Space via `hf` or repo settings (that label is not `OpenKotOR/<anything>`); treat **`openkotor-site.static.hf.space`** as the canonical Hub mirror unless using a [custom domain](https://huggingface.co/docs/hub/spaces-custom-domain).
