...
- Default Hub Space id is **`OldRepublicDevs/site`** → static app URL **`https://oldrepublicdevs-site.static.hf.space/`** (Hub hostnames are always `{org_slug}-{space_slug}.static.hf.space`). The HF static build sets **`VITE_HF_PUBLIC_BRAND=Old Republic Devs`** so the mirror matches the Hub org name; GitHub Pages builds omit it (default public name **OpenKotOR**).
