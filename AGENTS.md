...
- Default Hub Space id is **`OldRepublicDevs/site`** → static app URL **`https://oldrepublicdevs-site.static.hf.space/`** (Hub hostnames are always `{org_slug}-{space_slug}.static.hf.space`). **`https://openkotor.static.hf.space/`** is not assignable to a Space via `hf` or repo settings; use **`oldrepublicdevs-site.static.hf.space`** as the canonical Hub mirror unless using a [custom domain](https://huggingface.co/docs/hub/spaces-custom-domain).
